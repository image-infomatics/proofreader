"""
COPIED FROM https://github.com/tiangexiang/CurveNet/tree/main/core/models
...
class ParallelCurveNet(nn.Module):
    def __init__(self, num_channels, num_classes=2, k=20, num_points=2048, setting='default'):
        super(ParallelCurveNet, self).__init__()

        self.Embeders = nn.ModuleList(
            [EmbedCurve(k=k, num_classes=2, setting=setting) for i in range(num_channels)])
...
class EmbedCurve(nn.Module):
    def __init__(self, num_classes=2, k=20, setting='default'):
        super(EmbedCurve, self).__init__()
        ...
        self.cic11 = CIC(npoint=1024, radius=0.05, k=k, in_channels=additional_channel,
                         output_channels=64, bottleneck_ratio=2, mlp_num=1, curve_config=curve_config[setting][0])
...
class LPFA(nn.Module):
    ...
    def group_feature(self, x, xyz, idx):
        ...
        point_feature = point_feature.view(
            batch_size, num_points, self.k, -1)  # bs, n, k, 3
        points = xyz.view(batch_size, num_points, 1,
                          3).expand(-1, -1, self.k, -1)  # bs, n, k, 3
        ...
        feature = feature.view(batch_size, num_points,
                               self.k, num_dims)  # bs, n, k, c
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if self.att is not None:
            psix, psig = self.att(
                interpolated_points.permute(0, 2, 1), points1)
...
class CIC(nn.Module):
    ...
    def forward(self, xyz, x):
        ...
        if self.use_curve:
            # curve grouping
            curves = self.curvegrouping(
                x, xyz, idx[:, :, 1:])  # avoid self-loop
...
class CurveAggregation(nn.Module):
    ...
    def forward(self, x, curves):
        ...
        curver_inter = torch.sum(
            curves * F.softmax(curves_att, dim=-1), dim=-1)  # bs, c, c_n
        curves_intra = torch.sum(
            curves * F.softmax(curves_att, dim=-2), dim=-2)  # bs, c, c_l
        ...
        x_inter = F.softmax(
            torch.bmm(x_logits, curver_inter), dim=-1)  # bs, n, c_n
        x_intra = F.softmax(
            torch.bmm(x_logits, curves_intra), dim=-1)  # bs, l, c_l
        ...
        curve_features = torch.cat(
            (x_inter, x_intra), dim=-1).transpose(1, 2).contiguous()
...
class MaskedMaxPool(nn.Module):
    ...
    def forward(self, xyz, features):
        ...
        sub_features = F.max_pool2d(
            neighborhood_features, kernel_size=[
                1, neighborhood_features.shape[3]]
        )  # bs, c, n, 1
...
class Walk(nn.Module):
    ...
    def forward(self, xyz, x, adj, cur):
        ...
        tmp_adj = (adj + batch_offset.view(-1, 1, 1)
                   ).view(adj.size(0)*adj.size(1), -1)  # bs, n, k
        ...
        for step in range(self.curve_length):
            ...
            if step == 0:
                ...
                pre_feature = starting_points.view(
                    bn, self.curve_num, -1, 1).transpose(1, 2)  # bs * n, c
            else:
                # dynamic momentum
                ...
                att_feature = F.softmax(self.momentum_mlp(cat_feature), dim=1).view(
                    bn, 1, self.curve_num, 2)  # bs, 1, n, 2
                cat_feature = torch.cat(
                    (cur_feature, pre_feature), dim=-1)  # bs, c, n, 2

                # update curve descriptor
                pre_feature = torch.sum(
                    cat_feature * att_feature, dim=-1, keepdim=True)  # bs, c, n
                pre_feature_cos = pre_feature.transpose(
                    1, 2).contiguous().view(bn * self.curve_num, -1)
            ...
            pre_feature_expand = torch.cat(
                (pick_values, pre_feature_expand), dim=1)

            # which node to pick next?
            pre_feature_expand = self.agent_mlp(
                pre_feature_expand)  # bs, 1, n, k

            if step != 0:
                # cross over supression
                d = self.crossover_suppression(cur_feature_cos - pre_feature_cos,
                                               pick_values_cos -
                                               cur_feature_cos.unsqueeze(-1),
                                               bn, self.curve_num, self.k)
                d = d.view(bn, self.curve_num, self.k).unsqueeze(
                    1)  # bs, 1, n, k
                pre_feature_expand = torch.mul(pre_feature_expand, d)

            pre_feature_expand = gumbel_softmax(
                pre_feature_expand, -1)  # bs, 1, n, k

            cur_feature = torch.sum(
                pick_values * pre_feature_expand, dim=-1, keepdim=True)  # bs, c, n, 1

            cur_feature_cos = cur_feature.transpose(
                1, 2).contiguous().view(bn * self.curve_num, c)

            cur = torch.argmax(pre_feature_expand, dim=-
                               1).view(-1, 1)  # bs * n, 1

            flatten_cur = batched_index_select(
                pick_idx, 1, cur).squeeze()  # bs * n
